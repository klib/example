...
static void Parent_Main( KLObj* pObj, void* pParam ){
	...
	{
		p->x = klib.device.w*0.5f;
		p->y = klib.device.h*0.25f*(p->id+1);
	}
	else
	{
		p->x = klib.device.w*0.25f*(p->id+1);
		p->y = klib.device.h*0.5f;
	}
...
static void ObjContainer_FadeOutEnd( KLObj* pObj, void* pParam ){
...
static void ObjContainer_Update( KLObj* pObj, void* pParam ){
	...
	KL_DrawRect( klib.device.w*0.5f-72, klib.device.h-144, 144, 144, RGBA(0,0,0,255) );
				
	if( KLTouch_IsStart( klib.device.w*0.5f-72, klib.device.h-144, 144, 144 ) )
	{
	...
	{
		KLScene_FadeOut( ObjContainer_FadeOutEnd, pObj, RGBA(0,0,0,0), 0);
	}
...
void ObjContainer_Init( KLObj* pObj, void* pParam ){
	...
	KLLabel_ToSprite( &p->label, "黒い矩形をタップで全親から子を発生\n(子はMAX8)\n親タップですべての子を消滅させる\n端末向き変えで描画方向も変わる", &font );
	KLFont_Unload(&font);
